...
| MDS matrix | `Poseidon/Spec.lean` | Concrete 3×3 Cauchy matrix from Orchard spec |
| MDS bijectivity | `Poseidon/Properties.lean` | MDS matrix is invertible — proven via explicit inverse |
...
The only remaining axiom is `roundConstants` — the 192 round constants generated by the Grain LFSR. These are opaque data, not a mathematical claim.

All security-relevant properties (S-box bijectivity, MDS invertibility, round bijectivity, permutation bijectivity) are **fully proven**:
